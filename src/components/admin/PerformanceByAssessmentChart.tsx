...
import { AssessmentPerformanceData } from "./analytics-types";

interface Props {
  data: AssessmentPerformanceData[];
}

export function PerformanceByAssessmentChart({ data }: Props) {
  ...
  if (!hasData) {
    return (
      <div className="flex h-72 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card p-6 text-center shadow-xs">
        <LayoutTemplate className="size-8 text-muted-foreground mb-2" />
  ...
  return (
    <div className="space-y-4 rounded-xl border border-border bg-card p-5 shadow-xs">
      <div>
        <h3 className="text-sm font-bold text-foreground">Performance by Assessment Template</h3>
        <p className="text-xs text-muted-foreground">Average candidate score achieved across each curriculum template</p>
      </div>
      ...
      <div className="h-72 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 25 }}>
            ...
            <XAxis dataKey="templateTitle" tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} interval={0} angle={-15} textAnchor="end" />
      ...
      <div className="divide-y divide-border border-t border-border pt-2 text-xs">
        {data.map((item) => (
          <div key={item.templateTitle} className="flex justify-between py-1.5 text-muted-foreground">
            <span className="font-medium text-foreground">{item.templateTitle}</span>
