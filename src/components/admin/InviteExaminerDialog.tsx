...
export function InviteExaminerDialog({ open, onOpenChange, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (open) {
      setFullName("");
      setEmail("");
      setSpecialisations(["Light Vehicle Mechanical"]);
      setCustomSpec("");
      setMaxCandidates("20");
      setLoading(false);
      setErrorMsg(null);
  ...
  const addSpec = (item: string) => {
    const val = item.trim();
    if (val && !specialisations.includes(val))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await inviteExaminer(
      fullName,
      email,
      specialisations,
    ...
    if (res.error) {
      ...
      setEmailWarning(res.emailWarning || null);
    }
  };
        ...
        {temporaryPassword ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
            ...
            <div>
              ...
              <input
                type="email"
                placeholder="sarah.jenkins@skills.edu.au"
                value={email}
            ...
            <div>
              ...
              <div className="flex gap-1">
                <input
                  ...
                  className="flex-1 rounded border border-border bg-background p-1.5 text-xs"
                />
                <Button
                  type="button"
                  variant="outline"
              ...
              <div className="flex flex-wrap gap-1 mt-1.5">
                {COMMON_SPECIALISATIONS.filter(
                  (s) => !specialisations.includes(s),
                )
                  .slice(0, 3)
                  .map((cs) => (
                    <button
                      ...
                      type="button"
                      onClick={() => addSpec(cs)}
                      className="text-[10px] px-1.5 py-0.5 rounded border border-dashed border-border text-muted-foreground hover:text-foreground cursor-pointer"
                    >
                      + {cs}
                    </button>
                  ))}
            ...
            <DialogFooter className="gap-2 sm:gap-0 pt-2 border-t border-border">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <Mail className="size-3.5 mr-1" /> Create Examiner
                  </>
                )}
              </Button>
            </DialogFooter>
