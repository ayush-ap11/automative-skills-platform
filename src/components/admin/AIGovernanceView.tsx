  ...
  const filtered = useMemo(
    () =>
      analyses.filter((item) => {
        if (reviewFilter === "reviewed" && !item.is_human_reviewed)
          return false;
        if (reviewFilter === "pending" && item.is_human_reviewed) return false;
        if (flagFilter === "flagged" && !item.critical_safety_flag)
          return false;
        if (flagFilter === "unflagged" && item.critical_safety_flag)
          return false;
        return true;
      }),
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl border border-border bg-card shadow-xs space-y-2 text-xs">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            ...
            <p className="text-muted-foreground text-[11px]">
              Enforce anti-bias protocols across examiner evaluations and
              grading workflows.
            </p>
          </div>
          <Switch
        ...
        <p className="text-muted-foreground text-[11px] pt-1">
          When enabled, examiners see candidate answers and evidence without a
          name until that candidate&apos;s assessment is finalised.
        </p>
      </div>
      ...
      <div className="space-y-3 text-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h3 className="text-base font-bold text-foreground flex items-center gap-1.5">
              <Bot className="size-4 text-primary" /> AI Transparency &amp;
              Oversight Log
            </h3>
            <p className="text-muted-foreground text-[11px]">
              Audit trail of automated evaluation inferences, confidence
              metrics, and examiner reviews.
            </p>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex min-h-[200px] flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card p-8 text-center shadow-xs">
            <Bot className="size-8 text-muted-foreground mb-2" />
            <p className="text-xs text-muted-foreground">
              No AI-generated analyses match your current filters.
            </p>
          </div>
        ) : (
          <div className="divide-y divide-border rounded-xl border border-border bg-card overflow-hidden shadow-xs">
            <div className="hidden md:grid grid-cols-12 gap-3 p-3 bg-muted/40 font-semibold text-muted-foreground uppercase text-[10px] tracking-wider items-center">
              <div className="col-span-3">Candidate</div>
              <div className="col-span-4">Question</div>
              <div className="col-span-2">Model &amp; Confidence</div>
              <div className="col-span-3 text-right">
                <div>Human Review Status</div>
                <div className="text-[9px] font-normal text-muted-foreground normal-case tracking-normal">
                  Whether an examiner has confirmed this AI-generated score.
                </div>
              </div>
            </div>
            ...
            {filtered.map((item) => {
              const confDisplay = (() => {
                if (item.confidence_level == null) return "Not yet available";
                const num = Number(item.confidence_level);
              ...
              return (
                <div
                  key={item.id}
                  className="p-3.5 md:grid md:grid-cols-12 md:gap-3 md:items-center space-y-2 md:space-y-0 hover:bg-muted/30 transition-colors"
                >
                  ...
                  <div className="md:col-span-2 space-y-0.5">
                    <div className="font-mono text-[10px] text-foreground">
                      {item.model_version || "gemini-2.5-flash"}
                    </div>
                  ...
                  <div className="md:col-span-3 flex items-center justify-between md:justify-end gap-2">
                    ...
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${item.is_human_reviewed ? "bg-[var(--success)]/10 text-[var(--success)] border-[var(--success)]/20" : "bg-[var(--warning)]/10 text-[var(--warning)] border-[var(--warning)]/20"}`}
                    >
                      {item.is_human_reviewed ? (
                        <>
