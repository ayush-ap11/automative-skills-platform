...
import { Loader2, AlertCircle, Plus, X, ShieldAlert } from "lucide-react";
import { updateExaminer } from "@/app/(admin)/admin/examiners/actions";
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !loading && onOpenChange(v)}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Edit Examiner Profile</DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <DialogFooter className="gap-2 sm:gap-0 pt-2 border-t border-border">
            ...
            <Button type="submit" disabled={loading} className="cursor-pointer bg-[var(--primary)] text-[var(--primary-foreground)] hover:opacity-90">
              {loading ? <><Loader2 className="size-3.5 animate-spin mr-1.5" /> Saving...</> : "Save Changes"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
