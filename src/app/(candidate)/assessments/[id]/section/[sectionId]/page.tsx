...
export default async function SectionRunnerPage({ params }: SectionPageProps) {
  ...
  const { data: assessment } = await adminClient.from("assessments").select("id, status, template_id, candidate_profile_id").eq("id", id).single();
  if (!assessment || assessment.candidate_profile_id !== cp.id) redirect("/assessments");

  if (assessment.status === "not_started") {
    await adminClient.from("assessments").update({ status: "in_progress" }).eq("id", id);
  }

  const { data: section } = await adminClient.from("assessment_sections").select("id, title, template_id").eq("id", sectionId).eq("template_id", assessment.template_id).single();
  if (!section) redirect(`/assessments/${id}/take`);

  const { data: verbalCheck } = await adminClient.from("questions").select("id").eq("section_id", sectionId).eq("question_type", "verbal").limit(1);
  const isVerbal = (verbalCheck || []).length > 0;

  if (isVerbal) {
    const { data: rawVerbal } = await adminClient.from("questions").select("id, section_id, question_text, mandatory, time_limit_seconds").eq("section_id", sectionId).eq("question_type", "verbal").order("created_at", { ascending: true });
    ...
    if (verbalQuestions.length > 0) {
      const { data: caData } = await adminClient.from("candidate_answers").select("question_id, verbal_answers(audio_storage_path, duration_seconds, transcripts(transcript_text))").eq("assessment_id", id).in("question_id", verbalQuestions.map((q) => q.id));
  ...
  const { data: rawQuestions } = await adminClient.from("questions").select("id, section_id, question_text, question_type, mandatory, image_url, time_limit_seconds, question_options(id, option_text, order_index)").eq("section_id", sectionId).in("question_type", NON_VERBAL_TYPES).order("created_at", { ascending: true });
  ...
  if (questions.length > 0) {
    const { data: answers } = await adminClient.from("candidate_answers").select("question_id, selected_option_ids, answer_text").eq("assessment_id", id).in("question_id", questions.map((q) => q.id));
