...
export interface AdminDocumentDetailsResult {
  fileUrl?: string; category?: string; categoryLabel?: string; fileName?: string; isSensitive?: boolean;
...
async function verifyAdminDocAccess(docId: string) {
  ...
  const { data: doc } = await supabase.from("documents")
    .select("id, candidate_profile_id, category, storage_path, file_name, status, ai_extracted_data, is_sensitive, candidate_profiles!inner(profile_id, profiles!inner(organisation_id))")
    .eq("id", docId).maybeSingle();

  if (!doc) return { error: "Document not found" };
  const docOrgId = (doc as any).candidate_profiles?.profiles?.organisation_id;
  if (docOrgId !== profile.organisation_id) return { error: "Access denied: Document outside organization" };

  return { supabase, user, doc, candidateUserId: (doc as any).candidate_profiles?.profile_id };
}

export async function getAdminDocumentDetails(documentId: string): Promise<AdminDocumentDetailsResult> {
  const auth = await verifyAdminDocAccess(documentId);
  ...
  const pastReviews = (reviews || []).map((r: any) => ({
    ...
    reviewerName: r.reviewer?.preferred_name || r.reviewer?.full_name || "Reviewer",
  }));

  const isSens = Boolean(doc.is_sensitive || SENSITIVE_CATEGORIES.includes(doc.category));

  return {
    fileUrl: signed.signedUrl, category: doc.category,
    categoryLabel: CATEGORY_LABELS[doc.category] || doc.category,
    fileName: doc.file_name, isSensitive: isSens,
    aiExtractedData: doc.ai_extracted_data as Record<string, unknown> | null, pastReviews,
  };
}

export async function reviewDocumentAdmin(
  ...
  const { supabase, user, doc, candidateUserId } = auth;
  ...
  if (candidateUserId) {
    ...
    await supabase.from("notifications").insert({
      recipient_id: candidateUserId, type: "document_reviewed", title: "Document Reviewed",
      message: `Your ${catLabel} document has been reviewed by an administrator.`,
    });
  }
