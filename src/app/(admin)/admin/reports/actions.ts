...
export async function getAdminReportUrl(reportId: string): Promise<{ url?: string; error?: string }> {
  ...
  const { data: report } = await supabase
    .from("reports")
    .select("id, file_storage_path, candidate_profiles!inner(profiles!inner(organisation_id))")
    .eq("id", reportId)
    .maybeSingle();
  ...
  if (repOrgId !== orgId) return { error: "Access denied: Report outside organization" };

  if (!report.file_storage_path) {
    return { error: "Report file is not yet ready or storage path is missing." };
  }
  ...
  try {
    ...
    if (signErr || !signed?.signedUrl) {
      return { error: signErr?.message || "Failed to generate report download URL" };
    }
